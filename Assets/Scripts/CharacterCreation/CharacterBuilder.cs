...
public class CharacterBuilder : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Set the default value of the gender slider to male (0)
        genderSlider.value = 0;

        // Manually call PopulateSliders to create sliders for male on startup
        PopulateSliders();

        // Add listener after setting the default value to avoid triggering it immediately
    ...
    void LoadSlotDataForGender(string gender)
    {
        ...
        //Debug.Log($"Loaded {slots.Count} slots for {gender}: {String.Join(", ", slots)}");
    }
    ...
    void CreateSliderForSlot(string slotName)
    {
        ...
        // Get the Slider component from the prefab
    ...
    {
        Debug.Log($"Slider '{slotName}' changed to value: {value}");
    ...
    void LoadModelFromJson(string slotName, float value)
    {
        ...
        if (File.Exists(jsonFilePath))
        {
            ...
            if (slotModelData != null && slotModelData.meshes != null && slotModelData.meshes.Count > 0)
            {
                ...
                Debug.Log($"Loading model at index {modelIndex} for slot '{slotName}'");
                ...
                // Unload the current model for this slot, if any
                ...
                if (prefab != null)
                {
                    ...
                    currentlyLoadedModels[slotName] = modelInstance;  // Track the loaded model
                }
                else
                {
    ...
    void LoadPrefab(string modelName)
    {
        // The path should be relative to the Resources folder and without the .prefab extension
        string prefabPath = Path.Combine("Prefabs", modelName);
        GameObject prefab = Resources.Load<GameObject>(prefabPath);
